"""
Job search and discovery services for candidates.

Handles filtering, sorting, pagination context, and category listing.
"""

from django.db.models import Q, Count

from core.models import Application, Job, SavedJob


def get_job_filters(request):
    """Extract job filter parameters from GET request."""
    return {
...
def filter_jobs(jobs, filters):
    """Apply search/filter criteria to a Job queryset."""
    q = filters.get("q")
...
def get_job_context(user, page_obj):
    """Get saved/applied status for jobs on the current page."""
    saved_job_ids = set()
    applied_job_ids = set()
...
def get_categories():
    """Get distinct job categories for filter dropdowns."""
    return list(
